...
var collection3 = db.get('dailyworkers');
...
//workers in and out operations
router.post('/updatedata', function (req, res) {
collection3.findOne({"mobilenumber":req.body.mobilenumber,"date":moment().format("DD-MM-YYYY")}, function (err, docs1) {
	if (err || (docs1 == null)) {
		collection1.findOne({"mobilenumber":req.body.mobilenumber}, function (error, docs) {
    if(error || (docs == null)){
    	res.sendStatus(500)
    }
    else{
     var data = {
		name:docs.name,
		mobilenumber:docs.mobilenumber,
		aadhaarnumber:docs.aadhaarnumber,
		date:moment().format("DD-MM-YYYY"),
		gender:docs.gender,
		timein:moment().format("LTS"),
		//transport:req.body.transport,
		role:docs.role,
		address:docs.address,
		fileupload:docs.fileupload,
		status:"IN"
	}
	collection3.insert(data, function(error, docs){
		if (error) {
			res.sendStatus(500)
		} else {
			res.sendStatus(200)
...
//total workers proctor dashboard
...
collection3.find({"status":"IN","date":moment().format("DD-MM-YYYY")}, function(error, docs){
	if (error) {
		res.sendStatus(500)
	} else {
		res.send(docs)
...
collection3.find({"status":"OUT","date":moment().format("DD-MM-YYYY")}, function(error, docs){
	if (error) {
		res.sendStatus(500)
	} else {
		res.send(docs)
...
//upadting status
router.post('/workeroutdata', function(req, res){
	collection3.update({"_id":req.body._id},{$set:{"status":"OUT","timeout":moment().format("LTS")}}, function(error, docs){
		if (error) {
			res.sendStatus(500)
		} else {
			res.sendStatus(200)
...
//workersin data
router.get('/getdata1', function(req, res){
collection3.find({"status":"IN"}, function(error, docs){
	if (error) {
		res.sendStatus(500)
	} else {
		res.send(docs)
...
//workersout data
router.get('/getdata2', function(req, res){
collection3.find({"status":"OUT"}, function(error, docs){
	if (error) {
		res.sendStatus(500)
	} else {
		res.send(docs)
